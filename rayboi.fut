...
def check_sphere (ray: Vec) ({pos, radius, color}: Sphere): f32 =
    let sphere_center = pos in
    let s = f32.sqrt(((dot sphere_center ray) * (dot sphere_center ray)) - ((dot sphere_center sphere_center) - radius*radius)) in
    let d1 = (dot (-1 <*> sphere_center) ray) + s in
    let d2 = (dot (-1 <*> sphere_center) ray) - s in
    let d = f32.min d1 d2 in
    if f32.isnan d || d > 0 then f32.inf else d

def trace_ray (ray: Vec) (spheres: []Sphere): Vec =
...
def render (camera_orig: Vec, spheres: []Sphere): [height][width][3]f32 =
    ...
    let rays = (map (\y -> map3 (\x y z -> normalize {x = x, y = y, z = z}) xs (replicate width y) (replicate width (-1.0))) ys |> flatten) in
...
def main (x: f32) (y: f32) (z: f32): [height][width][3]f32 = (render({x = x, y = y, z = z}, [
    ...
    {pos = {x = 0, y = 0, z = -16}, radius = 2, color = {x=1, y=0, z=1}}
    ]))
